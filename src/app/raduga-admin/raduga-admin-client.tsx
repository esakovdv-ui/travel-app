...
import styles from './raduga-admin.module.css';
...
function normalizeShift(shift: RadugaShift): RadugaShift {
  return {
    ...shift,
    id: shift.id || makeId(),
...
export function RadugaAdminClient() {
  ...
  useEffect(() => {
    const saved = window.sessionStorage.getItem('raduga-admin-password');
    if (saved) {
      setPassword(saved);
      setIsAuthed(true);
    }
  ...
  useEffect(() => {
    ...
    async function loadShifts() {
      ...
      setError('');
      try {
        const response = await fetch('/api/raduga-shifts', { cache: 'no-store' });
  ...
  function login(event: FormEvent<HTMLFormElement>) {
    ...
    if (password !== 'raduga2026') {
      setError('Неверный пароль.');
      return;
    }

    window.sessionStorage.setItem('raduga-admin-password', password);
    setIsAuthed(true);
  }

  function logout() {
    window.sessionStorage.removeItem('raduga-admin-password');
    setIsAuthed(false);
    setPassword('');
    setShifts([]);
  }

  function updateShift(index: number, patch: Partial<RadugaShift>) {
  ...
  async function save() {
    ...
    try {
      const response = await fetch('/api/raduga-shifts', {
        method: 'POST',
    ...
    <main className={styles.page}>
      <div className={styles.shell}>
        <div className={styles.topbar}>
          <div>
            <h1 className={styles.title}>Смены ДОЛ «Радуга»</h1>
            <p className={styles.subtitle}>Редактируйте карточки смен, цены, изображения и ссылки на бронирование.</p>
          </div>
