...
| Лист | Что внутри |
|------|------------|
| **Визард** | Баннер/popup, шаги опроса, handoff + CR шаг→шаг |
| **Туры** | Handoff туры → выдача → карточка → корзина → бронь → заявка + CR |
| **Отели** | Handoff отели → выдача → корзина → чекаут → блок оплаты → оплата + CR |
| **Справочник** | ID целей и фильтры |

Строки — **недели**. В турах и отелях после каждого шага колонка **CR** = конверсия от предыдущего шага (формат `72,8%`).
