...
| Туры: выдача | 90662828 | Пользователи с `podbor_ref=1` или UTM (handoff сразу открывает search — это и есть выдача) |
...
В блоке **ТУРЫ** первая строка — handoff визарда, вторая — выдача на 90662828 по `podbor_ref`/UTM (без moduleId). После деплоя маркера цифры близки; разрыв = нажали кнопку, но страница туров не открылась.
